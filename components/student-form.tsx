...
const StudentForm = ({ studentData }: StudentDataProps) => {
  ...
  //@ts-expect-error suppressing ts error for setSubmitting type
  const handleSubmitAttendance = (values, setSubmitting) => {
  ...
  return (
    <SafeAreaView
      className="m-5 mt-20 items-center justify-center rounded-3xl  bg-white p-6"
      style={{
      ...
      <Formik
        ...
        onSubmit={(values, { setSubmitting }) => handleSubmitAttendance(values, setSubmitting)}>
        {({
          values,
          errors,
          touched,
